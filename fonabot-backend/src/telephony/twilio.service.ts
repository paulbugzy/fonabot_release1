...
export class TwilioTelephonyService implements ITelephonyService {
  ...
  async makeTestCall(userId: string, to: string, url: string): Promise<string> {
    try {
      const { client, phoneNumber } = await this.getTwilioClient(userId);
  ...
  async handleIncomingCall(callData: IncomingCallDto): Promise<string> {
    ...
    try {
      ...
      // Generate initial TwiML response, PASSING callSid
      return this.generateInitialTwiML(phoneNumber.assignedIvrFlow, callData.CallSid); // Pass callSid
  ...
  // Change signature
  private generateInitialTwiML(ivrFlow: IvrFlow, callSid: string): string {
    const response = new twiml.VoiceResponse();
    // Use the passed callSid
