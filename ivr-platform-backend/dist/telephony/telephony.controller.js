...
let TelephonyController = TelephonyController_1 = class TelephonyController {
    ...
    async handleIvrStep(callData) {
        ...
        if (callData.RecordingUrl && callData.useASR === 'true') {
            const asrResult = await this.callExecutionService.processExternalASR(callData.CallSid, callData.RecordingUrl);
            return this.callExecutionService.executeStep(callData.CallSid);
        }
        else {
            if (input) {
                await this.callExecutionService.updateSessionVariables(callData.CallSid, {
                    ...
                    last_speech_confidence: input.confidence,
                });
            }
    ...
    async makeTestCall(userId, dto) {
        const twimlUrl = dto.message;
        const callSid = await this.telephonyService.makeCall(userId, dto.phoneNumber, twimlUrl);
        return { callSid };
    }
};
